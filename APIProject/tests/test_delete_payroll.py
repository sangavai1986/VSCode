...
def test_delete_payroll(client_factory,payroll_id):
    payroll_client = client_factory(PayrollClient, "BASE_URL", os.getenv("BASE_URL"))
    response = payroll_client.delete_payroll(payroll_id)
    try:
        ...
        logging.info(f"TEST PASSED: Payroll Deleted Successfully for ID {payroll_id}")
