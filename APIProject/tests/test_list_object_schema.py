...
def test_list_object_schema(client_factory):
    ...
    schema = schema_values("list_object_schema.json")
    ...
    response = object_client.get_object(obj_id)
    try:
        validate(schema=schema,instance=response.json())
        logging.info("TEST PASSED:Schema Validation passed for an object listing")
